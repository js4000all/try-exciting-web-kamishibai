...
- **型同期方式**: シナリオ/セーブデータは `shared/schema/*.schema.json` を正として管理し、`server` と `web` は同一スキーマに依存する。
- **CI 型整合コマンド**: `./sync-and-check.sh` を CI の型整合チェックの単一コマンドとして実行する。
- **検出基準**: 片側（`server` または `web`）だけ型変更した場合、生成物差分チェックで CI が失敗する。
